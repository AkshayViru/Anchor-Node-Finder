...
while graph_count<10:
	...
	f_out= open("result100_%s_%sResult.txt"%(probab,graph_count),"w+")

	while found==False:
		G= nx.erdos_renyi_graph(limit, probab)

		if nx.is_connected(G):
			f_in= open("100_%s_%s.txt"%(probab, graph_count),"w+")
			nx.write_edgelist(G,"100_%s_%s.txt"%(probab, graph_count),data=False)
		...
		if __name__ == '__main__':
			...
			while count<4:
				...
				if cur_d<min_d:
					...
					node2=n2
				count+=1
			print (graph_count+1, ll_reqd, min_d, node1, node2)
