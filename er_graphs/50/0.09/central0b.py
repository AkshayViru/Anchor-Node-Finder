...
graph_num=1
probab=0.09
f_out= open("central0b.txt" ,"w+")

while graph_num<=10:
	G= nx.Graph()

	f = open('50_%s_%s.txt' %(probab,graph_num))
	...
	for i in range(50):
		...
		ltotal_dcentrality=[v for k, v in (nx.betweenness_centrality(G)).items() if k == i]		

		if ltotal_dcentrality[0]>=highest_dcentrality:
			highest_dcentrality=ltotal_dcentrality[0]
